...
export default function ReviewStats({ productId }) {
    ...
    >
      ...
      <div className="flex-grow-1">
        <div
          ...
          style={{ position: "relative", height: "1.5rem", marginBottom: "0.5rem" }}
        >
          {marks.map((text, i) => (
            <div
              key={i}
        ...
        <input
          ...
          max={marks.length - 1}
          value={(() => {
            if (label === "지속력") return stats.averageLongevity - 1;
            if (label === "확산력") return stats.averageSillage - 1;
            if (label === "성별")
  ...
  return (
    <div className="d-flex px-4 py-3 justify-content-between align-items-start" style={{ gap: "2rem" }}>
      {/* 왼쪽: 별점 평균 + 분포 */}
      <div style={{ flex: "1" }}>
        <div className="text-center mb-3">
          <div className="d-flex justify-content-center align-items-center mb-1">
            <span style={{ fontSize: "2rem", color: "#f5c518" }}>★</span>
            <span className="fw-bold" style={{ fontSize: "2rem", marginLeft: "8px" }}>
              {stats.averageRating}
            </span>
          </div>
        ...
        {distribution.map((count, idx) => (
          <div key={idx} className="d-flex align-items-center mb-2">
            ...
            <div
              ...
              style={{ height: "10px", borderRadius: "5px", overflow: "hidden" }}
            >
              <div
                style={{
      ...
      {/* 오른쪽: 슬라이더 */}
      <div style={{ flex: "2" }}>
        {renderSlider("지속력", stats.averageLongevity, ["매우약함", "약함", "중간", "강함", "아주강함"])}
        {renderSlider("확산력", stats.averageSillage, ["매우약함", "약함", "중간", "강함", "아주강함"])}
        {renderSlider("성별", stats.majorityGender, ["여성적", "중성적", "남성적"])}
      </div>
    </div>
  );
}